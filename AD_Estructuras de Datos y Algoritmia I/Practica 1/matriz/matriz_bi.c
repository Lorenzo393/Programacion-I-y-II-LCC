...
void matriz_intercambiar_filas(Matriz *matriz, int fila1, int fila2){
    ...
    double *aux = matriz->direccion[fila1];
    matriz->direccion[fila1] = matriz->direccion[fila2];
    matriz->direccion[fila2] = aux;
}
...
void matriz_insertar_fila(Matriz *matriz, int posFila){
    ...
    matriz->numFilas++;
    matriz->direccion = realloc(matriz->direccion, sizeof(double*) * matriz->numFilas);
    matriz->direccion[matriz->numFilas] = malloc(sizeof(double) * matriz->numColumnas);
    for(int i = matriz->numFilas ; i > posFila ; i--){
        matriz_intercambiar_filas(matriz, i, i-1);
    }
    for(int i = 0 ; i < matriz->numColumnas ; i++){
        matriz->direccion[posFila][i] = -1;
    }
}